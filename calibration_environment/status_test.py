...
class TestCheckStatus:
    ...
    def test_status_error_logged_and_raised_with_contents(
        ...
        mock_gas_mixer_status_check, mock_water_bath_status_check = mock_status_checks
    ...
    def test_multiple_exceptions_logged_and_returned(
        ...
        with pytest.raises(
            module.CalibrationSequenceAbort,
            match=r"GasMixerStatusError.*WaterBathStatusError",
        ):
            module.check_status(MOCK_PORTS)

        # logger.exception should get called once for each error
